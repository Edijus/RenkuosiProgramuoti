...
def show_orders(request, page=1):
    orders_per_page = 1
    orders = Order.objects.annotate(order_detail_price=F('orderdetail__quantity') * F('orderdetail__service__price')) \
...
def show_user_orders(request, page=1):
    orders_per_page = 1
    orders = Order.objects.filter(user=request.user.id).annotate(order_detail_price=F('orderdetail__quantity') * F('orderdetail__service__price')) \
        .annotate(total_price=Sum('order_detail_price')).order_by('-date_created') \
        .values('id', 'image', 'date_created', 'total_price', 'vehicle')
    paginator = Paginator(orders, orders_per_page)
    paginated_orders = paginator.get_page(page)
