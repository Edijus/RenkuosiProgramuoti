...
login_manager.login_view = 'sign_in'  # Jei neprisijungę mėginsim eit, kur reikia prisijungimo, redirektins į login puslapį
...
class MyModelView(ModelView):
    def is_accessible(self):
        return current_user.is_admin  # Calculate users, if 1, then it is admin
...
def sign_in():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            login_user(user)
            flash(f'Welcome, {current_user.first_name}', 'success')
            return redirect(request.args.get('next') or url_for('home')) # Nukreips į NEXT URI nurodytą puslapį
